...
function fontResource(name: string) {
  return {
    name,
...
function makeEditor() {
  ...
  const editor = {
    ...
    host: { t: (k: string) => k, listFonts: (...a: unknown[]) => listFonts(...a) },
    fontStore: {
      builtins: vi.fn(() => [
        { id: 'sans', name: 'Sans' },
        { id: 'serif', name: 'Serif' },
...
describe('useTextEditPopup', () => {
  ...
  it('appends host resource fonts to the options', async () => {
    listFonts.mockResolvedValue([fontResource('CoolFont')])
    const { editor } = makeEditor()
    const api = useTextEditPopup(editor, ref(null))
    await flush()
    expect(listFonts).toHaveBeenCalled()
    expect(api.fontOptions.value).toEqual([
      { label: 'Sans', value: 'builtin:sans' },
      { label: 'Serif', value: 'builtin:serif' },
  ...
  it('keeps only builtin options when the resource list fails', async () => {
    listFonts.mockRejectedValue(new Error('offline'))
    const { editor } = makeEditor()
    const api = useTextEditPopup(editor, ref(null))
    await flush()
    expect(api.fontOptions.value).toEqual([
