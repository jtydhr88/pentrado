  ...
  >
    <div class="ctv:flex ctv:items-center ctv:gap-1">
      <span class="ctv:flex-1 ctv:text-3xs ctv:uppercase ctv:tracking-wide ctv:text-muted-foreground">
        {{ $t('pentrado.sectionText') }}
      </span>
      <button type="button" :class="miniBtnClass" :title="$t('pentrado.close')" @click="close">
    ...
    <textarea
      ...
      :value="layer.text"
      rows="2"
      :placeholder="$t('pentrado.textPlaceholder')"
    ...
    <div class="ctv:flex ctv:items-center ctv:gap-1.5">
      <span :class="labelClass">{{ $t('pentrado.font') }}</span>
    ...
    <div v-if="fontFailed" class="ctv:text-2xs ctv:text-red-400">
      {{ $t('pentrado.loadFontFailed') }}
    </div>

    <div class="ctv:flex ctv:items-center ctv:gap-1.5">
      <span :class="labelClass">{{ $t('pentrado.fontSize') }}</span>
      <input
      ...
      />
      <span :class="labelClass">{{ $t('pentrado.textColor') }}</span>
      <input
        type="color"
    ...
    <div class="ctv:flex ctv:items-center ctv:gap-1.5">
      <span :class="labelClass">{{ $t('pentrado.letterSpacing') }}</span>
      <input
      ...
      />
      <span :class="labelClass">{{ $t('pentrado.lineHeight') }}</span>
      <input
    ...
    <div class="ctv:flex ctv:items-center ctv:gap-1.5">
      <span :class="labelClass">{{ $t('pentrado.align') }}</span>
...
const ALIGNS = [
  { value: 'left' as const, labelKey: 'pentrado.alignLeft', icon: IconAlignLeft },
  { value: 'center' as const, labelKey: 'pentrado.alignCenter', icon: IconAlignCenter },
  { value: 'right' as const, labelKey: 'pentrado.alignRight', icon: IconAlignRight },
]
