...
describe('extractPatch', () => {
  ...
  it('handles a full-buffer rect as an identity copy', () => {
    ...
    expect([...extractPatch(src, 2, { x: 0, y: 0, w: 2, h: 1 })]).toEqual([...src])
  })
})

describe('SetContentRegionCommand', () => {
  const rect = { x: 1, y: 1, w: 2, h: 2 }
  const patch = (v: number) => new Uint8ClampedArray(rect.w * rect.h * 4).fill(v)
  ...
  it('budget cost is the patches, not the full layer', () => {
    ...
    const cmd = new SetContentRegionCommand('Paint', { contentId: after }, patches(), store)
  ...
  it('multiple scattered patches sum instead of spanning their bounding box', () => {
    ...
    const two = [
      ...
      { rect: { x: 1000, y: 1000, w: 2, h: 2 }, before: patch(0), after: patch(1) },
    ]
    const cmd = new SetContentRegionCommand('Paint', { contentId: after }, two, store)
  ...
  it('undo/redo rebuild fresh content entries instead of swapping ids', () => {
    const store = new DefaultContentStore()
    const after = store.register(canvas(8, 8))
    const slot: { contentId: string; url?: string } = { contentId: after, url: undefined }
    const cmd = new SetContentRegionCommand('Paint', slot, patches(), store, 'http://x/a.png')
