...
import {
  borderMask, combineMasks, ellipseMask, emptyMask, featherMask, floodSelectMask, growMask,
  maskBoundary, maskBounds, polygonMask, rectMask, shrinkMask, type FloodSource, type GrayMask,
} from './selectionMath'

function maskOf(width: number, height: number, on: Array<[number, number]>): GrayMask {
...
describe('combineMasks (GIMP gimp-gegl-mask-combine semantics)', () => {
  const base = maskOf(4, 1, [[0, 0], [1, 0]])
  const addOn = maskOf(4, 1, [[1, 0], [2, 0]])
  ...
  it('subtract is clamped difference', () => {
    expect([...combineMasks(base, addOn, 'subtract').data]).toEqual([1, 0, 0, 0])
  })
  it('intersect is min', () => {
    expect([...combineMasks(base, addOn, 'intersect').data]).toEqual([0, 1, 0, 0])
  })
})
...
describe('shape masks', () => {
  ...
  it('polygonMask rasterizes a triangle', () => {
    const m = polygonMask(10, 10, [{ x: 1, y: 1 }, { x: 9, y: 1 }, { x: 1, y: 9 }])
...
describe('floodSelectMask (GIMP scanline segment flood)', () => {
  function source(width: number, height: number, painter: (x: number, y: number) => [number, number, number]): FloodSource {
  ...
  it('U-shaped region floods around the barrier', () => {
    const src = source(5, 5, (x, y) => (x === 2 && y < 4 ? [255, 255, 255] : [0, 0, 0]))
  ...
  it('non-contiguous mode matches the whole image by color', () => {
    const src = source(6, 1, (x) => (x % 2 === 0 ? [10, 10, 10] : [200, 200, 200]))
