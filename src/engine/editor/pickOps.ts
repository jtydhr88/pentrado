...
export type AlphaSampler = (canvas: HTMLCanvasElement, x: number, y: number) => number

const sampleCache = new WeakMap<HTMLCanvasElement, CanvasRenderingContext2D | null>()

export function defaultAlphaSampler(canvas: HTMLCanvasElement, x: number, y: number): number {
...
function rasterAlphaAt(
  node: RasterData, pt: Vec2, content: ContentStore, sample: AlphaSampler,
): number {
...
export function layerOpacityAt(
  node: SceneNode, pt: Vec2, content: ContentStore,
...
export function pickLayerAt(
  layers: readonly SceneNode[], pt: Vec2, content: ContentStore,
  sample: AlphaSampler = defaultAlphaSampler,
): SceneNode | null {
  for (let i = layers.length - 1; i >= 0; i--) {
    const node = layers[i]
    if (layerOpacityAt(node, pt, content, sample) > PICK_OPACITY_THRESHOLD) return node
  }
  return null
}
