...
function mkNode(id: string, x: number, y: number, w: number, h: number): SceneNode {
  return {
    id, kind: 'raster', name: id, visible: true, opacity: 1,
    mode: { blend: 'normal', blendSpace: 'auto', compositeSpace: 'auto', composite: 'union', legacy: false },
...
function mkRoot(children: SceneNode[]): GroupData {
  return {
    id: 'root', kind: 'group', name: 'root', visible: true, opacity: 1,
    mode: { blend: 'normal', blendSpace: 'auto', compositeSpace: 'auto', composite: 'union', legacy: false },
