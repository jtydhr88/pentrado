...
import { Dirty, type History } from '../history'

function copyGuides(doc: Document): DocGuide[] {
  return (doc.guides ?? []).map((g) => ({ ...g }))
}

function makeCommand(doc: Document, label: string, before: DocGuide[], after: DocGuide[]): PropCommand<DocGuide[]> {
  return new PropCommand<DocGuide[]>(
    label, Dirty.META,
    () => copyGuides(doc),
    (v) => { doc.guides = v.length ? v.map((g) => ({ ...g })) : undefined },
    before, after,
  )
}

export function sanitizeGuides(raw: unknown, width: number, height: number): DocGuide[] {
...
export function guideAddLive(doc: Document, axis: 'x' | 'y', pos: number): number {
...
export function guideEndDrag(
  doc: Document, history: History, index: number, end: GuideDragEnd,
): void {
