...
import { CommandGroup, type Command, type History } from '../history'
...
export function arrangeNodes(
  root: GroupData, ids: string[], op: ArrangeOp, history: History,
): boolean {
  ...
  for (let i = 0; i < targets.length; i++) {
    ...
    cmds.push(new SetTransformCommand('arrange', node, before, { ...node.transform }))
  }
