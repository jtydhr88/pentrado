...
function makeProgram(gl: WebGL2RenderingContext, vs: WebGLShader, fragSrc: string): WebGLProgram {
...
function ensureTargets(fx: FxGpu, w: number, h: number): void {
  const gl = fx.gl
  if (fx.targetSize.w === w && fx.targetSize.h === h && fx.targets.length === 4) return
...
function loc(gl: WebGL2RenderingContext, prog: WebGLProgram, name: string): WebGLUniformLocation | null {
...
export function applyLayerFxChainGpu(
  ...
  try {
    ...
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, fx.staging)
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.NEAREST)
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.NEAREST)
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE)
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE)
    ...
    const bindInput = (prog: WebGLProgram, name: string, tex: WebGLTexture, unit: number) => {
    ...
    for (const f of active) {
      ...
      if (f.op === 'gaussian-blur') {
        ...
        gl.uniform2f(loc(gl, prog, 'u_offset'), (f.params.x ?? 0) / w, (f.params.y ?? 0) / h)
        draw()
        out = t.tex
      } else {
        const OP = { vignette: 0, noise: 1, desaturate: 2, pixelate: 3, emboss: 4 } as const
        ...
        const p = f.params
        const packed =
          f.op === 'vignette' ? [p.radius ?? 1.2, Math.max(0.001, p.softness ?? 0.8), Math.max(0.01, p.gamma ?? 1), 0]
          : f.op === 'noise' ? [p.amount ?? 0.2, 0, 0, 0]
          : f.op === 'desaturate' ? [Math.max(0, Math.min(1, p.amount ?? 1)), 0, 0, 0]
          : f.op === 'pixelate' ? [Math.max(2, Math.round(p.size ?? 8)), 0, 0, 0]
          : [p.azimuth ?? 30, p.elevation ?? 45, Math.max(1, p.depth ?? 20), 0]
        gl.uniform4f(loc(gl, prog, 'u_p'), packed[0], packed[1], packed[2], packed[3])
        draw()
        out = t.tex
      }
