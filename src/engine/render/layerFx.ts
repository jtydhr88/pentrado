...
function blurChannelPass(src: Float32Array, dst: Float32Array, w: number, h: number, r: number): void {
  const norm = 1 / (2 * r + 1)
  for (let y = 0; y < h; y++) {
    const row = y * w
    let acc = 0
    ...
    for (let x = 0; x < w; x++) {
      dst[row + x] = acc * norm
      acc += src[row + Math.max(0, Math.min(w - 1, x + r + 1))] - src[row + Math.max(0, Math.min(w - 1, x - r))]
    }
  }
}
...
function gaussianBlurPremul(img: ImageData, sigma: number): ImageData {
  ...
  for (const c of ch) {
    ...
    for (let y = 0; y < h; y++) for (let x = 0; x < w; x++) tr[x * h + y] = c[y * w + x]
    for (const size of sizes) {
      const r = Math.round((size - 1) / 2)
      if (r < 1) continue
    ...
    for (let y = 0; y < h; y++) for (let x = 0; x < w; x++) c[y * w + x] = tr[x * h + y]
  }
  ...
  for (let p = 0; p < n; p++) {
    ...
    out.data[p * 4 + 1] = Math.round(Math.max(0, Math.min(1, ch[1][p] * inv)) * 255)
    out.data[p * 4 + 2] = Math.round(Math.max(0, Math.min(1, ch[2][p] * inv)) * 255)
  }
  return out
}

function applyDropShadow(img: ImageData, params: Record<string, number>): ImageData {
  const w = img.width
  const h = img.height
  ...
  const blurred = (params.stdDev ?? 0) > 0 ? gaussianBlurPremul(shadow, params.stdDev) : shadow
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      ...
      out.data[i + 1] = Math.round((img.data[i + 1] * fa + bg * ba * (1 - fa)) / outA)
      out.data[i + 2] = Math.round((img.data[i + 2] * fa + bb * ba * (1 - fa)) / outA)
...
function applyMedian(img: ImageData, radius: number): ImageData {
  ...
  for (let dy = -r; dy <= r; dy++) widths[dy + r] = Math.round(Math.sqrt(Math.max(0, r * r - dy * dy)))
  const out = new ImageData(w, h)
  const hist = [new Int32Array(256), new Int32Array(256), new Int32Array(256), new Int32Array(256)]
...
function applyEmboss(img: ImageData, params: Record<string, number>): ImageData {
  const w = img.width
  const h = img.height
  ...
  for (let p = 0; p < w * h; p++) {
    luma[p] = 0.2126 * img.data[p * 4] + 0.7152 * img.data[p * 4 + 1] + 0.0722 * img.data[p * 4 + 2]
  }
  const out = new ImageData(w, h)
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
...
function applyOne(img: ImageData, f: LayerFxData): ImageData {
  switch (f.op) {
    case 'gaussian-blur':
      return (f.params.stdDev ?? 0) > 0 ? gaussianBlurPremul(img, f.params.stdDev) : img
    case 'unsharp-mask': {
      const blurred = gaussianBlurPremul(img, Math.max(0.5, f.params.stdDev ?? 3))
      const scale = f.params.scale ?? 0.5
      const out = new ImageData(img.width, img.height)
      for (let p = 0; p < img.width * img.height; p++) {
        const i = p * 4
        for (let c = 0; c < 3; c++) {
          out.data[i + c] = Math.max(0, Math.min(255, img.data[i + c] + (img.data[i + c] - blurred.data[i + c]) * scale))
        }
        out.data[i + 3] = img.data[i + 3]
      }
      return out
    ...
    case 'desaturate': {
      ...
      for (let p = 0; p < img.width * img.height; p++) {
        ...
        img.data[i + 1] = Math.round(img.data[i + 1] + (l - img.data[i + 1]) * amt)
        img.data[i + 2] = Math.round(img.data[i + 2] + (l - img.data[i + 2]) * amt)
      }
      return img
    }
  }
...
export function applyLayerFxChain(bitmap: Bitmap, fx: LayerFxData[]): { canvas: HTMLCanvasElement; pad: number } | null {
